...
class Producer
{
    ...
    {
        return $this->queue->send($message, $metadata, $delayOrTimestamp);
    }
     ...
     * 
     * @param array $messages 消息数组，支持 delay 参数控制延时发送
    ...
    public function sendBatch(array $messages): array
    {
        ...
        foreach ($messages as $message) {
            if (is_array($message)) {
                ...
                $delayOrTimestamp = $message['delay'] ?? 0;
                $results[] = $this->send($content, $metadata, $delayOrTimestamp);
            } else {
                // 处理简单消息
